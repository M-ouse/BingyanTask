...
<html>
	<head>
		<title>User Listing | Management System</title>
...
User Listing | Inventory Management System
                </div>
            </div>
        </nav>
				...
				<tr>
					...
					<td>
						<select name="f_ugroup">
							...
							<option value="2" <?php echo $f_ugroup == "2" ? "selected" : "" ?>>Visitor</option>
						</select>
					</td>
				</tr>
				<tr>
		...
		<table>
			...
			if ($result_user_lst->num_rows > 0) {				
				while($row = $result_user_lst->fetch_assoc()){
					...
					if($row["ugroup"] == "0"){$usergroup = 'Admin';}
					if($row["ugroup"] == "1"){$usergroup = 'Staff';}
					if($row["ugroup"] == "2"){$usergroup = 'Visitor';}
					echo 
					"
					<tr>
